...
ROOT_CERT_URL="http://127.0.0.1:8080/config/root-ca-cert"
MQTT_CRED_URL="https://127.0.0.1:8443/config/sensor-credentials"
HOST_BASE=150000
RSP_FILE_BASE="rsp_"
...
get_mqtt_credentials () {
    ...
    curl --insecure --header "Content-type: application/json" --request POST --data '{"username":"'$1'","token":"'$2'","generatedTimestamp":'$TIME',"expirationTimestamp":-1}' $MQTT_CRED_URL
}

# Function takes device_id and rsp index as arguments
send_connect_request () {
    mosquitto_pub -t rfid/rsp/connect -m '{"jsonrpc":"2.0","id":"1","method":"connect","params":{"hostname":"'$1'","hwaddress":"98:4f:ee:15:00:'$2'","app_version":"19.1.sim","module_version":"none","num_physical_ports":2,"motion_sensor":true,"camera":false,"wireless":false,"configuration_state":"unknown","operational_state":"unknown"}}'
}

# Function takes device_id and facility_id as arguments
send_status_indication () {
    TIME=$(($(date +%s%N)/1000000))
    mosquitto_pub -t rfid/rsp/rsp_status/$1 -m '{"jsonrpc":"2.0","method":"status_update","params":{"sent_on":'$TIME',"device_id":"'$1'","facility_id":"'$2'","status":"ready"}}'
}

# Function takes device_id and facility_id as arguments
send_heartbeat_indication () {
    TIME=$(($(date +%s%N)/1000000))
    mosquitto_pub -t rfid/rsp/rsp_status/$1 -m '{"jsonrpc":"2.0","method":"heartbeat","params":{"sent_on":'$TIME',"device_id":"'$1'","facility_id":"'$2'","location":null,"video_url":null}}'
}
...
send_device_alert_indication () {
    TIME=$(($(date +%s%N)/1000000))
    mosquitto_pub -t rfid/rsp/rsp_status/$1 -m '{"jsonrpc":"2.0","method":"device_alert","params":{"sent_on":'$TIME',"device_id":"'$1'","facility_id":"'$2'","alert_number":'$3',"alert_description":"'$4'","severity":"'$5'","optional":'$6'}}'
}

# Function takes device_id and facility_id as arguments
send_inventory_complete_indication () {
    TIME=$(($(date +%s%N)/1000000))
    mosquitto_pub -t rfid/rsp/rsp_status/$1 -m '{"jsonrpc":"2.0","method":"inventory_complete","params":{"sent_on":'$TIME',"device_id":"'$1'","facility_id":"'$2'"}}'
}

# Function takes device_id, facility_id and EPC as arguments
send_inventory_data_indication () {
    TIME=$(($(date +%s%N)/1000000))
    mosquitto_pub -t rfid/rsp/data/$1 -m '{"jsonrpc":"2.0","method":"inventory_data","params":{"sent_on":'$TIME',"period":500,"device_id":"'$1'","facility_id":"'$2'","location":null,"motion_detected":true,"data":[{"epc":"'$3'","tid":null,"antenna_id":0,"last_read_on":'$TIME',"rssi":-654,"phase":32,"frequency":915250}]}}'
}

# Function takes device_id and "id":id as arguments
send_command_response () {
    mosquitto_pub -t rfid/rsp/response/$1 -m '{"jsonrpc":"2.0","result":true,'$2'}'
}

# Function takes device_id, uptime and "id":id as arguments
send_bist_results_response () {
    mosquitto_pub -t rfid/rsp/response/$1 -m '{"jsonrpc":"2.0","result":{"rf_module_error":false,"rf_status_code":0,"rf_module_temp":'$RFID_TEMP',"ambient_temp":'$AMBI_TEMP',"time_alive":'$2',"cpu_usage":'$CPU_USAGE',"mem_used_percent":'$MEM_USAGE',"mem_total_bytes":'$MEM_TOTAL',"camera_installed":false,"temp_sensor_installed":true,"accelerometer_installed":true,"region":"USA","rf_port_statuses":[{"port":0,"forward_power_dbm10":280,"reverse_power_dbm10":0,"connected":true},{"port":0,"forward_power_dbm10":280,"reverse_power_dbm10":0,"connected":true}],"device_moved":false},'$3'}'
}

# Function takes device_id, index and "id":id as arguments
send_state_response () {
    mosquitto_pub -t rfid/rsp/response/$1 -m '{"jsonrpc":"2.0","result":{"device_id":"'$1'","hwaddress":"98:4f:ee:15:00:'$2'","app_version":"19.1.sim","module_version":"none","num_physical_ports":2,"motion_sensor":true,"camera":false,"wireless":false,"configuration_state":"unknown","operational_state":"unknown"},'$3'}'
}

# Function takes device_id and "id":id as arguments
send_sw_version_response () {
    mosquitto_pub -t rfid/rsp/response/$1 -m '{"jsonrpc":"2.0","result":{"app_version":"19.1.sim","module_version":"none"},'$2'}'
}
...
wait_for_connect_response () {
    ...
    MSG=$(mosquitto_sub -t rfid/rsp/connect/${rsp[$DEVICE_ID_INDEX]} -C 1)
...
wait_for_command () {
    ...
    while [ 0 -lt 1 ]; do
        ...
        CMD=$(mosquitto_sub -t rfid/rsp/command/${rsp[$DEVICE_ID_INDEX]} -C 1)
